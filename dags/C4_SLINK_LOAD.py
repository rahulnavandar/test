...
ENVIRONMENTS = {
    'pilot': {
        'dag-name': 'C4_SLINK_USAGE_DAILY_DWH',
        'connection-id': 'springer',
        'project': bqproject,
        'bucket': 'usagec4raw',
        'dataflow-template': 'gs://sprna-dev-ldz-stg-usage/C4UsageFile'
    }
}


# example {"days": ["01-08-2019","02-08-2019"] } or  { "days": ["daily_process"] }
backlogdays_toprocess = os.environ.get("slink_backlogdays_toprocess")
...
dag = DAG(
    ...
    catchup=False,
    description='Load Slink Usage data'
)
...
for day in daystoprocess:
    ...
    seldel_command = "bq query --nouse_legacy_sql 'delete " + '`' + bqproject + "." + datasetenv + "_STG_Usage.Usage_C4_" + yearmonth + "`" + " WHERE UsageCalenderDate = \"" + date + "\"'" + " && " + \
                     "bq query --nouse_legacy_sql 'delete " + '`' + bqproject + "." + datasetenv + "_DWH_Usage.Usage_C4_Slink_BP_Base_Reporting_Daily` WHERE UsageCalenderDate = \"" + date + "\"'" + " && " + \
                     "bq query --nouse_legacy_sql 'delete " + '`' + bqproject + "." + datasetenv + "_DWH_Usage.Usage_C4_Slink_Material_Base_Reporting_Daily` WHERE UsageCalenderDate = \"" + date + "\"'"
    ...
    dataflow_conversion = DataflowTemplateOperator(
        ...
        dataflow_default_options={
            'project': ENVIRONMENTS['pilot']['project'],
            'zone': 'europe-west1-d',
            'tempLocation': 'gs://sprna-dev-ldz-stg-usage/temp/'
        },
    ...
    move_data_to_bigquery_stg = GoogleCloudStorageToBigQueryOperator(
        ...
        source_objects=[(
            ...
            "%Y%m%d") + '.log*')],
        destination_project_dataset_table=datasetenv + '_STG_Usage.Usage_C4_' + day_format.strftime("%Y%m"),
        source_format='NEWLINE_DELIMITED_JSON',
        schema_object='usageschema.json',
    ...
    LoadBP = BigQueryOperator(
        dag=dag,
        task_id=taskid,
        sql='Usage/SlinkUsage/C4_DWH_BPReporting.sql',
        params={"project": bqproject, "environment": datasetenv, "yearmonth": yearmonth,
                "date": date},
        allow_large_results=True,
        write_disposition='WRITE_APPEND',
        use_legacy_sql=False,
        destination_dataset_table=bqproject + "." + datasetenv + '_DWH_Usage.Usage_C4_Slink_BP_Base_Reporting_Daily',
        bigquery_conn_id=ENVIRONMENTS['pilot']['connection-id']
    )
    ...
    LoadMaterial = BigQueryOperator(
        dag=dag,
        task_id=taskid,
        sql='Usage/SlinkUsage/C4_DWH_MaterialReporting.sql',
        params={"project": bqproject, "environment": datasetenv, "yearmonth": yearmonth, "date": date},
        allow_large_results=True,
        write_disposition='WRITE_APPEND',
        use_legacy_sql=False,
        destination_dataset_table=bqproject + "." + datasetenv + '_DWH_Usage.Usage_C4_Slink_Material_Base_Reporting_Daily',
        bigquery_conn_id=ENVIRONMENTS['pilot']['connection-id']
    )
    ...
    sel_del_bp_monthly = BigQueryOperator(
        dag=dag,
        task_id=taskid,
        sql='Usage/SlinkUsage/C4_SelDeletion_BP_Monthly.sql',
    ...
    sel_del_mat_monthly = BigQueryOperator(
        dag=dag,
        task_id=taskid,
        sql='Usage/SlinkUsage/C4_SelDeletion_Mat_Monthly.sql',
        params={"project": bqproject, "environment": datasetenv, "yearmonthsep": yearmonthsep},
        write_disposition='WRITE_APPEND',
        use_legacy_sql=False,
        bigquery_conn_id=ENVIRONMENTS['pilot']['connection-id']
    ...
    RefreshMonthlyAggrBPReporting = BigQueryOperator(
        dag=dag,
        task_id=taskid,
        sql='Usage/SlinkUsage/C4_Refresh_BP_MonthlyAggregate.sql',
        params={"project": bqproject, "environment": datasetenv, "yearmonthsep": yearmonthsep},
        write_disposition='WRITE_APPEND',
        use_legacy_sql=False,
        destination_dataset_table=bqproject + "." + datasetenv + '_DWH_Usage.Usage_C4_Slink_BP_Base_Reporting_Monthly',
        bigquery_conn_id=ENVIRONMENTS['pilot']['connection-id']
    )
    ...
    RefreshMonthlyAggrMaterialReporting = BigQueryOperator(
        dag=dag,
        task_id=taskid,
        sql='Usage/SlinkUsage/C4_Refresh_Material_MonthlyAggregate.sql',
        params={"project": bqproject, "environment": datasetenv,"yearmonthsep": yearmonthsep},
        write_disposition='WRITE_APPEND',
        use_legacy_sql=False,
        destination_dataset_table=bqproject + "." + datasetenv + '_DWH_Usage.Usage_C4_Slink_Material_Base_Reporting_Monthly',
        bigquery_conn_id=ENVIRONMENTS['pilot']['connection-id']
    )
    ...
    #     dag=dag,
    #     task_id=taskid,
    #     sql='Usage/SlinkUsage/C4_MergeBP.sql',
    #     params={"project": bqproject, "environment": datasetenv, "yearmonth": yearmonth, "date": date},
    #     write_disposition='WRITE_APPEND',
    #     use_legacy_sql=False,
    #     bigquery_conn_id=ENVIRONMENTS['pilot']['connection-id']
    ...
    #     dag=dag,
    #     task_id=taskid,
    #     sql='Usage/SlinkUsage/C4_MergeArticle.sql',
    #     params={"project": bqproject, "environment": datasetenv, "yearmonth": yearmonth, "date": date},
    #     write_disposition='WRITE_APPEND',
    #     use_legacy_sql=False,
    #     bigquery_conn_id=ENVIRONMENTS['pilot']['connection-id']
    ...
    #     dag=dag,
    #     task_id=taskid,
    #     sql='Usage/SlinkUsage/C4_MergeChapter.sql',
    #     params={"project": bqproject, "environment": datasetenv, "yearmonth": yearmonth, "date": date},
    #     write_disposition='WRITE_APPEND',
    #     use_legacy_sql=False,
    #     bigquery_conn_id=ENVIRONMENTS['pilot']['connection-id']
